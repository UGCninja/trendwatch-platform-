...
async def api_test_ig_likers(request: Request, url: str):
    ...
    async with httpx.AsyncClient(timeout=140) as client:
        for input_fmt in [
            {"posts": [url], "max_count": 5},
            {"posts": [{"url": url}], "max_count": 5},
        ]:
            try:
                r = await client.post(
                    "https://api.apify.com/v2/acts/datadoping~instagram-likes-scraper/run-sync-get-dataset-items",
...
# ══════════════════════════════════════════════════════════════
#  ACCOUNTS  —  одиночный аккаунт → авто-сбор 50 постов
# ══════════════════════════════════════════════════════════════
...
def _extract_account_handle(url: str, platform: str) -> str:
    """Извлекаем username/handle из URL аккаунта."""
    import re as _re2
    u = url.strip().rstrip("/")
    if platform == "TikTok":
        m = _re2.search(r'tiktok\.com/@([^/?]+)', u)
        return m.group(1) if m else ""
    if platform == "Instagram":
        m = _re2.search(r'instagram\.com/([^/?]+)', u)
        h = m.group(1) if m else ""
        return h if h not in ("p", "reel", "tv", "stories") else ""
    if platform == "YouTube":
        m = _re2.search(r'youtube\.com/(?:@|channel/|c/)([^/?]+)', u)
        return m.group(1) if m else ""
    if platform == "X":
        m = _re2.search(r'(?:x|twitter)\.com/([^/?]+)', u)
        return m.group(1) if m else ""
    return ""


async def _fetch_account_posts(account_url: str, platform: str, sc_key: str, yt_key: str, limit: int = 50) -> list[str]:
    """Возвращает список URL последних N постов аккаунта."""
    handle = _extract_account_handle(account_url, platform)
    if not handle:
        return []
    urls: list[str] = []
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            if platform == "TikTok":
                r = await client.get(
                    "https://api.scrapecreators.com/v2/tiktok/user/posts",
                ...
                if r.status_code == 200:
                    ...
                    for item in items[:limit]:
                        url = (item.get("webVideoUrl") or item.get("video_url") or
                               item.get("url") or
                               (f"https://www.tiktok.com/@{handle}/video/{item['aweme_id']}" if item.get("aweme_id") else ""))
                        if url:
                            urls.append(url)
                ...
                if r.status_code == 200:
                    ...
                    for item in items[:limit]:
                        ...
                        url = item.get("url") or item.get("link") or (f"https://www.instagram.com/p/{shortcode}/" if shortcode else "")
                        if url:
                            urls.append(url)
                ...
                if r.status_code == 200:
                    ...
                    for item in items[:limit]:
                        ...
                        if tid:
                            urls.append(f"https://x.com/{handle}/status/{tid}")
    except Exception:
        pass
    return urls
...
async def accounts_list(request: Request):
    ...
    from app.database import SessionLocal
    db = SessionLocal()
    accounts = db.query(_AccountProject).order_by(_AccountProject.created_at.desc()).all()
    db.close()
    return templates.TemplateResponse(request=request, name="account_projects.html",
                                      context={"active_page": "accounts", "accounts": accounts})
...
async def accounts_create(request: Request, name: str = Form(...), account_url: str = Form(...)):
    ...
    from app.database import SessionLocal
    db = SessionLocal()
    platform = _detect_platform_from_url(account_url)
    # Создаём CommentProject-обёртку
    cp = CommentProject(name=f"[Account] {name}")
    db.add(cp)
    db.flush()
    ap = _AccountProject(name=name, account_url=account_url.strip(),
                         platform=platform, comment_project_id=cp.id)
    db.add(ap)
    db.commit()
    aid = ap.id
    db.close()
...
async def account_detail(request: Request, aid: int):
    ...
    # Переадресуем на detail comment_project
    pid = ap.comment_project_id
    db.close()
    return RedirectResponse(f"/accounts/{aid}/detail", status_code=302)


_account_tasks: dict[str, dict] = {}


@app.post("/accounts/{aid}/fetch-posts")
async def account_fetch_posts(request: Request, aid: int):
    ...
    urls = await _fetch_account_posts(ap.account_url, ap.platform or "",
                                      SCRAPECREATORS_API_KEY, YOUTUBE_API_KEY, limit=50)
    added = 0
    for url in urls:
        try:
            src = CommentSource(project_id=pid, url=url, platform=ap.platform or _detect_platform(url),
                                creator=_extract_account_handle(ap.account_url, ap.platform or ""))
            db.add(src)
            db.flush()
            added += 1
        except Exception:
...
@app.get("/accounts/{aid}/detail", response_class=HTMLResponse)
async def account_detail_page(request: Request, aid: int):
    if not check_auth(request):
        return RedirectResponse("/login", status_code=302)
    from app.database import SessionLocal
    db = SessionLocal()
    ...
    if not ap:
        db.close()
        return RedirectResponse("/accounts", status_code=302)
    # Используем ту же логику что и comment_project_detail
    pid = ap.comment_project_id
    db.close()
    # Делегируем в comment_project_detail с кастомным контекстом
    return await _account_detail_render(request, aid, pid)


async def _account_detail_render(request: Request, aid: int, pid: int):
    from app.database import SessionLocal
    from app.models import StoredComment as _SC2, StoredLiker as _SL2
    ...
    total_comments = db.query(_SC2).filter(_SC2.source_id.in_(src_ids)).count() if src_ids else 0
    total_likers = db.query(_SL2.username).filter(_SL2.source_id.in_(src_ids)).distinct().count() if src_ids else 0

    author_rows = []
    if src_ids:
        author_rows = (db.query(_SC2.author, _f2.count(_SC2.id).label("cnt"), _f2.max(_SC2.platform).label("platform"))
            .filter(_SC2.source_id.in_(src_ids), _SC2.author != None, _SC2.author != "")
            .group_by(_SC2.author).order_by(_f2.count(_SC2.id).desc()).limit(100).all())
    top_commenters = [(a, c, p) for a, c, p in author_rows]

    liker_per_source = {}
    if src_ids:
        for sid, cnt in db.query(_SL2.source_id, _f2.count(_SL2.id)).filter(_SL2.source_id.in_(src_ids)).group_by(_SL2.source_id).all():
            liker_per_source[sid] = cnt

    lang_rows = []
    if src_ids:
        lang_rows = (db.query(_SC2.language, _f2.count(_SC2.id))
    ...
    region_rows = []
    if src_ids:
        region_rows = (db.query(_SC2.user_region, _f2.count(_SC2.id))
            .filter(_SC2.source_id.in_(src_ids), _SC2.user_region != None, _SC2.user_region != "")
            .group_by(_SC2.user_region).order_by(_f2.count(_SC2.id).desc()).limit(20).all())
    region_stats = [(r, c) for r, c in region_rows]

    top_likers_rows = []
    if src_ids:
        top_likers_rows = (db.query(_SL2.username, _f2.count(_f2.distinct(_SL2.source_id)).label("posts"))
            .filter(_SL2.source_id.in_(src_ids))
            .group_by(_SL2.username).order_by(_f2.count(_f2.distinct(_SL2.source_id)).desc()).limit(100).all())
    top_likers = [(u, c) for u, c in top_likers_rows]

    provider_stats_raw: dict = {}
    for s in sources:
        key = s.creator or "—"
        if key not in provider_stats_raw:
            provider_stats_raw[key] = {"posts": 0, "comments": 0, "active": 0, "deleted": 0, "unavailable": 0}
        provider_stats_raw[key]["posts"] += 1
        provider_stats_raw[key]["comments"] += s.comments_count or 0
        if s.status == "active": provider_stats_raw[key]["active"] += 1
        elif s.status == "deleted": provider_stats_raw[key]["deleted"] += 1
        else: provider_stats_raw[key]["unavailable"] += 1
    provider_stats = list(provider_stats_raw.items())
    ...
    return templates.TemplateResponse(request=request, name="account_project_detail.html", context={
        ...
        "region_stats": region_stats,
        "liker_region_stats": [],
        "lang_region_stats_comments": [],
        "liker_per_source": liker_per_source,
        "provider_stats": provider_stats,
        "providers": list({s.creator for s in sources if s.creator}),
    })


@app.post("/accounts/{aid}/run")
async def account_run(request: Request, aid: int):
    ...
    if not pid:
        return RedirectResponse("/accounts", status_code=302)
    task_id = str(uuid.uuid4())
    _comments_tasks[task_id] = {"status": "queued", "done": 0, "total": 0,
                                 "ts": time.time(), "project_id": pid}
    threading.Thread(target=_run_project_comments_task,
                     args=[task_id, pid, SCRAPECREATORS_API_KEY, APIFY_TOKEN],
                     daemon=True).start()
    return RedirectResponse(f"/accounts/{aid}/detail", status_code=302)
...
async def account_delete(request: Request, aid: int):
    if not check_auth(request):
        return RedirectResponse("/login", status_code=302)
    from app.database import SessionLocal
    from app.models import StoredLiker as _SL3, StoredComment as _SC3
    db = SessionLocal()
    ap = db.query(_AccountProject).filter(_AccountProject.id == aid).first()
    if ap:
        pid = ap.comment_project_id
        ...
        if src_ids:
            db.query(_SL3).filter(_SL3.source_id.in_(src_ids)).delete(synchronize_session=False)
            db.query(_SC3).filter(_SC3.source_id.in_(src_ids)).delete(synchronize_session=False)
            db.query(CommentSource).filter(CommentSource.project_id == pid).delete(synchronize_session=False)
        db.query(CommentProject).filter(CommentProject.id == pid).delete(synchronize_session=False)
